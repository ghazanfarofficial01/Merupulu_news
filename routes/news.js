...
newsRouter.get("/api/news", async(req, res) => {
  try{  
    if(!req.query.category){
       const news = await News.find({}).sort({publishedAt:-1}).exec();
      //console.log(news)
      res.status(200).json(news)
  }
  else{
    const news = await News.find({category: req.query.category}).sort({publishedAt:-1}).exec();
      res.status(200).json(news)
  }
  }catch(e){
    res.status(500).json({ error: e.message });
...
newsRouter.get("/api/news/isBreaking", async(req, res) => {
  try{  
     const news = await News.find({isBreaking: true}).sort({publishedAt:-1}).exec();
      res.status(200).json(news)
